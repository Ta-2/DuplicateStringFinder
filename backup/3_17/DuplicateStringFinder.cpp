...
class DuplicateStringFinder{
	...
	void SearchDuplicateString(double limen){
		...
		*/
		...
		//AppearMap[(length if string)][vrctor<int>(location of appearance)]
		//-------------------------------------------------
		//print AppearMao
		//-------------------------------------------------
		...
		*/
		...
		//-------------------------------------------------
		//#define pair<const string, vector<int>> refPSV
		//map<int, vector<refPSV>>
		...
		cout << "<< Craeting PeriodMap2 >>" << endl;
		map<int, vector<refPSV>*> PeriodMap2;
		for(auto a: AppearMap){
			int WordCount = a.first, NumAppearance = a.second[0].second.size();
			double score = log2(WordCount) * NumAppearance;
			...
			PeriodMap2[a.first] = new vector<refPSV>;
			vector<refPSV>* AddrefPSV = PeriodMap2[a.first];
			for(auto s: a.second){
		...
		cout << "<< Print PeriodMap2 >>" << endl;
		for(auto p: PeriodMap2){
		...
		//map<int, vector<refPSV>>
		//map<int, vector<refPSV>*> PeriodMap2;
		cout << endl << "<< PRIORITY MAP by PeriodMap2 >>" << endl;
		for(auto p: PeriodMap2){
			cout << "--length of string: " <<  p.first << endl;
			for(auto q: *(p.second)){
				int min = *min_element(q.second.begin(), q.second.end());
				cout << "----string: \"" << q.first << "\"" << endl;
				cout << "------min: " << min << endl;
				cout << "------priority: ";
				for(int i = 1; i*i <= min; i++){
					int num_front = 0, num_back = 0;
